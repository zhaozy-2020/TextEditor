...
def New__(name = '',title = 'Untitled'):
    ...
    def color_f(event = None):
        ...
        def col(t):
            return '#%02X%02X%02X' %(int(t[0]), int(t[1]), int(t[2]))
        text.configure(froeground =  col(fileName[0]))
        lebel_f.configure(froeground =  col(fileName[0]))
    #toolbox
    fr = Frame(toll)
    ...
    color_bg = Button(toll,text = '背景颜色',command = color_b)
    color_fg = Button(toll,text = '文字颜色',command = color_f)
    lebel_b = Label(toll,background = 'white')
    lebel_f = Label(toll,background = 'black')
...
#文本框
    ...
    zt = Label(frame,text = 'ln:1 co:0',background = 'white',foreground = 'black')
    zt.pack(side = "Button",fill ='both')
    ...
    close.grid(row = 0,column = 7,sticky = W+E+N+S)
    yOrn.grid(row = 1,column = 7,sticky = W+E+N+S)
    color_bg.grid(row = 0,column = 8,sticky = W+E+N+S,pady = 5)
    color_fg.grid(row = 0,column = 9,sticky = W+E+N+S,pady =5)
    lebel_b.grid(row = 1,column = 8,sticky = W+E+N+S,pady = 5)
    lebel_f.grid(row = 1,column = 9,sticky = W+E+N+S,pady = 5)
    ztl()
#创建页签对象
New__()
