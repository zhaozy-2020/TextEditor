from tkinter import *
from tkinter.ttk import Notebook,Combobox,OptionMenu
...
def New__(name = '',title = 'Untitled'):
    ...
    def sizeslen(evend = None):
        nonlocal sizeVar,familyVar,weightVar
        try:
            f = Font(frame,size = sizeVar.get(),weight = weightVar.get(),family = familyVar.get())
            text.tag_add('big',SEL_FIRST,SEL_LAST)
            text.tag_config('big',font = f)
        except:
            f = Font(frame,family = familyVar.get(),weight = weightVar.get(),size = sizeVar.get())
    ...
    size.current(4) 
    size.bind_all('<<ComboboxSelected>>',sizeslen)
...
#文本框
    ...
    TheTextFrame.pack(fill = BOTH,expand = TRUE)
    ...
    popopmenu.add_separator()
    popopmenu.add_command(lebel = '')
    text.image_create(END,PhotoImage('F:/哈哈.png'))
